...
class MimeType(StrPrimitive):
    """Кастомный примитив для валидации MIME-тип файла"""

    from .value_objects import FileType  # noqa: PLC0415

    # Регулярное выражение для валидации MIME-типа согласно RFC 6838
    MIME_PATTERN: ClassVar[re.Pattern] = re.compile(r"^[a-z]+/[a-z0-9\-+._]+$", re.IGNORECASE)
    FILE_TYPE_MAP: ClassVar[dict[str, FileType]] = {
    ...
    def type(self) -> FileType:
        """Основная часть MIME-типа (до /)"""
        return self.FILE_TYPE_MAP.get(self.split("/")[0], self.FileType.OTHER)

    @property
    def subtype(self) -> str:
        """Подтип MIME-типа (после /)"""
        return self.split("/")[1]

    @classmethod
    def validate(cls, mime_type: str) -> str:
        if not bool(cls.MIME_PATTERN.match(mime_type)):
            raise ValueError(f"Mime type {mime_type} is not valid!")
        return mime_type
